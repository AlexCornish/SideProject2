...
def createCustomFormattedDataFrame(dataFrame,inputArray):
    ...
    if labelAdd == True:
        ...
        dataFrame.insert((dataFrame.columns.get_loc("value")+(int(percentageChg==True))+(int(seasonColumn==True))+1),"group_code",groupCode,True)
        # Attachs the item code to the dataframe
        dataFrame.insert((dataFrame.columns.get_loc("value")+(int(percentageChg==True))+(int(seasonColumn==True))+2),"item_code",itemCode,True)
